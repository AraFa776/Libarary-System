...
public class DbSeeder
{
    public static async Task SeedDefaultData(IServiceProvider service)
    {
        try
        {
            ...
            var adminEmails = new[]
               {
                 "ahmedh223344ssan@gmail.com",
                 "abdelrahmanarafa27@gmail.com",
                 "shoshotalaat031@gmail.com"
               };

            foreach (var email in adminEmails)
            {
                var adminUser = await userMgr.FindByEmailAsync(email);
                if (adminUser != null && !await userMgr.IsInRoleAsync(adminUser, Roles.Admin.ToString()))
                {
